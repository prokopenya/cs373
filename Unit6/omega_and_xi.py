...
class matrix:
    ...
    def __add__(self, other):
        ...
        if self.dimx != other.dimx or self.dimx != other.dimx:
            ...
            mu = matrix()
            mu.zero(self.dimx, self.dimy)
            for i in range(self.dimx):
                for j in range(self.dimy):
                    mu.value[i][j] = self.value[i][j] + other.value[i][j]
            return mu
    ...
    def __mul__(self, other):
        ...
        if self.dimy != other.dimx:
            ...
            mu = matrix()
            mu.zero(self.dimx, other.dimy)
            for i in range(self.dimx):
                for j in range(other.dimy):
                    for k in range(self.dimy):
                        mu.value[i][j] += self.value[i][k] * other.value[k][j]
        return mu
    ...
    def transpose(self):
        # compute transpose
        mu = matrix()
        mu.zero(self.dimy, self.dimx)
        for i in range(self.dimx):
            for j in range(self.dimy):
                mu.value[j][i] = self.value[i][j]
        return mu
    ...
    def take(self, list1, list2 = []):
        ...
        mu = matrix()
        mu.zero(len(list1), len(list2))
        for i in range(len(list1)):
            for j in range(len(list2)):
                mu.value[i][j] = self.value[list1[i]][list2[j]]
        return mu
    ...
    def expand(self, dimx, dimy, list1, list2 = []):
        ...
        for i in range(len(list1)):
            for j in range(len(list2)):
                mu.value[list1[i]][list2[j]] = self.value[i][j]
        return mu
    ...
    # This code is based on http://adorio-muearch.org/wordpmus/?p=4560

    def Cholesky(self, ztol= 1.0e-5):
        mu = matrix()
        mu.zero(self.dimx, self.dimx)

        for i in range(self.dimx):
            S = sum([(mu.value[k][i])**2 for k in range(i)])
            d = self.value[i][i] - S
            if abs(d) < ztol:
                mu.value[i][i] = 0.0
            else:
            ...
            for j in range(i+1, self.dimx):
                S = sum([mu.value[k][i] * mu.value[k][j] for k in range(i)])
                if abs(S) < ztol:
                    S = 0.0
                mu.value[i][j] = (self.value[i][j] - S)/mu.value[i][i]
        return mu
    ...
    # Backward step for inverse.
        for j in reversed(range(self.dimx)):
            tjj = self.value[j][j]
            S = sum([self.value[j][k]*mu.value[j][k] for k in range(j+1, self.dimx)])
            mu.value[j][j] = 1.0/ tjj**2 - S/ tjj
            for i in reversed(range(j)):
                mu.value[j][i] = mu.value[i][j] = \
                    -sum([self.value[i][k]*mu.value[k][j] for k in \
